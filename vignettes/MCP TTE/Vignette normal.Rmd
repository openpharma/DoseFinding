...
and focus on the placebo group and the four dose groups (12.5, 25, 50, and 100Î¼g).

For the purpose here,
we recreate a dataset that mimics a parallel group design, based on the published summary statistics.
...
ggplot(NVA) + 
  geom_jitter(aes(dose, FEV1), height = 0, width = 4) +
...
maFitMod aus DoseFinding package

```{r, bootstrap_draw}
fit_mod_av <- maFitMod(dose, mu_hat, S = S_hat,
                       models = c("emax", "sigEmax", "quadratic")) # how do I know how many iterations the bootstrap did????????????????

plot(fit_mod_av)
```

With the `predict` method we can obtain the predictions from the
fitted model on each bootstrap sample. The `plot` method allows to summarize
