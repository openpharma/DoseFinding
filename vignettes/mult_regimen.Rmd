...
od and bid, all patients would receive two administrations per
day (patients in the od group receive one placebo per day), so that
there is no real od group, and no placebo od group.  The placebo
group is then assumed common to both the od and bid dose-response
curve. For the MCP-step contrasts for both od and bid are taken
...
Also note that we decide to model everything on the total daily dose scale.
...
We will use the same candidate models for both regimen.
...
that is detailed in the [basic usage vignette](analysis_normal.html#dose-response-estimation).
