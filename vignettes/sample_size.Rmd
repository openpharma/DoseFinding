...
model is not among the chosen candidate shapes? Usually MCP-Mod is
quite robust.  To illustrate this, let's assume an exponential model
...
take forward into Phase 3 and establishment of a trend over placebo is
...
Here we only give a very brief example from the book:
Consider the `sigEmax(30.5, 3.5)` model from the first section
...
Now we can ask the question what the average length of a 90% confidence interval for TD would be.
...
```{r, tdci93}
# TODO: this gives a lot of warnings and sometimes NA results
set.seed(42)
...
```
The expected length of the CI (`lengthTDCI`) is larger than the true TD value.
This is clearly unsatisfactory.
Experimenting with different values of `n`,
one quickly realizes that we would need for example 1650 patients to get the expected length of the CI down to 20 mg.
```{r, tdci1650}
pm <- planMod("sigEmax", Mods(sigEmax=c(30.5, 3.5), placEff=1.25, maxEff=0.15, doses=doses),
...
In practice, to keep the size of
