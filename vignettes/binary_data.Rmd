...
ma_pred <- predict(fit_mod_av, doseSeq = dose_seq)
med <- apply(ma_pred, 2, median)
quants <- apply(ma_pred, 2, quantile, probs = c(0.025, 0.975))
ma_pred <- data.frame(dose_seq = dose_seq, median = med, low = quants[1,], high = quants[2,])
ma_summary <- as.data.frame(inv_logit(ma_pred)) # back to probability scale
ma_summary$dose_seq <- dose_seq
...
ggplot(ma_summary) + geom_line(aes(dose_seq, median)) +
    geom_ribbon(aes(x = dose_seq, ymin = low, ymax = high), alpha = 0.2) +
...
fit_cov_adj <- maFitMod(doses, ca$mu_star, S = ca$S_star,
                        models = c("emax", "sigEmax", "betaMod"))
# plotting on probability scale, need to transform predictions on logit scale
ma_pred <- predict(fit_cov_adj, doseSeq = dose_seq)
med <- apply(ma_pred, 2, median)
quants <- apply(ma_pred, 2, quantile, probs = c(0.025, 0.975))
ma_pred <- data.frame(dose_seq = dose_seq, median = med, low = quants[1,], high = quants[2,])
ma_summary <- as.data.frame(inv_logit(ma_pred)) # back to probability scale
