...
sim <- function(mods, model, doses, ratio, n, etotal) {
  ...
  entry <- runif(n, 0, qexp(etotal/n, lambda0)) # staggered entry (like in Paper Rühl 2022)
  event <- entry + rexp(length(mean_resp), exp(mean_resp)) # calendar time
...
```{r}
# here: for all models
S_and_S0 <- function(y, lambda0, ratio, ngroup, etotal) {
  # second column of beta / for second model here
  beta <- y - log(lambda0) # placebo adjusted log hazard rates
...
```{r}
S <- S_and_S0(y, lambda0, ratio, ngroup, etotal)$S
S0 <- S_and_S0(y, lambda0, ratio, ngroup, etotal)$S0
alpha <- 0.05

contMat <- optContr(mods, doses = doses_treat, S = S[,, 1], placAdj = T)[[1]]
```

powMCT1 from deng2019power:
cov2cor() scales a covariance matrix by its diagonal to become the correlation matrix\
\
powMCT1short is a modification of powMCT1 of Deng 2019.
...
powMCT1short <-
  function(mods,
    ...
    # S and S0
    sim <- S_and_S0(y, lambda0, ratio, ngroup, etotal)
...
```{r}
power_calc <- powMCT1short(mods, contMat, alpha, y, lambda0, ratio, ngroup, etotal = 30)
...
```{r}
pow_sep <- function(mods, contMat, alpha, y, lambda0, ratio, ngroup, des_pow){
  pow <- 0
  etotal_col <- c(0)
  pow_col <- c(0)
  ...
  for (i in 1:1000){
    etotal <- i
    pow <- powMCT1short(mods, contMat, alpha, y, lambda0, ratio, ngroup, 
                        etotal)$power_av
    etotal_col[i] <- i
    pow_col[i] <- pow
...
```{r, fig.width = 6.9, out.width = '100%'}
des_pow <- 0.7
test <- pow_sep(mods, contMat, alpha, y, lambda0, ratio, ngroup, des_pow)
test$etotal
test$graf
```
...
sample_size <- function(ratio, y, end_rec, end_stu, model, n){
  ngroup <- length(ratio)
  lambda <- exp(y)[,model]
  rat <- ratio/sum(ratio)
  D <- rep(0, ngroup)
...
```{r}
sample_size(ratio, y, end_rec = 1, end_stu = 3, model = 2, n = 100) 
```
...
```{r}
pow_sep2 <- function(mods, contMat, alpha, y, lambda0, ratio, des_pow, end_rec, 
                     end_stu, model, n_mind = 20){
  ngroup <- length(ratio)
  pow <- 0
  n_col <- c(0)
  pow_col <- c(0)
  ...
  for (i in (n_mind : 1000)){
    n <- i
    etotal_int <- floor(sample_size(ratio, y, end_rec, end_stu, model, 
                                    n = n)$etotal_samp) # round with floor????
    pow <- powMCT1short(mods, contMat, alpha, y, lambda0, ratio, ngroup, 
                        etotal = etotal_int)$power_av
    n_col[i - n_mind] <- i
  ...
  graf <- ggplot(data = dat, aes(x = n_col, y = pow_col, group = 1)) +
  geom_line()+
  geom_point() +
  ...
  return(list(n = n, etotal = etotal, graf = graf, data = dat))
}
...
```{r, fig.width = 6.9, out.width = '100%'}
pow_sep2(mods, contMat, alpha, y, lambda0, ratio, des_pow = 0.7, end_rec = 1, 
