...
predict_and_plot <- function(mu_hat, S_hat, doses, dose_seq, n_rep) {
  ...
  gg <- ggplot(cbind(bs_summary, dose_seq = dose_seq)) + geom_line(aes(dose_seq, median)) +
    ...
    geom_point(aes(dose, mu_hat), glm_summary, alpha = 0.5) +
...
\begin{aligned}
  Y_{kj} &\sim \mathrm{Bernoulli}(p_{kj}) \\
  \mathrm{logit}(p_{kj}) &= \mu_k^d + x_{kj}^T\mu^a
\end{aligned}
\]

Fitting this model gives us estimated coefficients $\hat\mu=(\hat\mu^d, \hat\mu^a)$
...
and the corresponding sub-matrix of $\hat S$.
To accommodate potential systematic differences between the covariates across the groups we go a slightly more complex route.
We predict the mean response probability at dose k for the observed values of the covariates and transform back to logit scale:
\[
\mu^*_k := \mathrm{logit}\biggl(\frac{1}{n} \sum_{j=1}^n
\mathrm{logit}^{-1}(\hat\mu^d_k + x_{jk}^T\hat\mu^a)\biggr)
\]
...
with Jeffrey's prior on the parameter.
...
For calculating the optimal contrast for the generalized MCP step
the covariance matrix $S$ of the estimator $\hat\mu$ needs to be re-estimated once the trial data are available.
With normally distributed data this is possible with decent accuracy even at low sample sizes.
In the case of binary data,
$\hat\mu$ is on the logit scale
and the diagonal elements of $S$ are approximately $(np(1-p))^{-1}$,
where $n$ is the sample size of the dose group.
This can be derived using the delta method.
An estimate of this variance depends on the observed response rate
and can be quite variable even for moderate sample sizes.

Such an estimation takes place in the calls of `MCTtest()` above.
As the estimate can get imprecise for small sample sizes per group (e.g. smaller than 20),
it may make sense no to estimate a variance at all in these situations.
One can just use a constant that is proportional to the sample size per dose.
The contrast calculated this way will asymptotically not be equal to the "optimal" contrast for the underlying model,
but simulations show that they can be closer to the "true" optimal contrast
(calculated based on the true variance per dose group)
for small sample size.

When re-running the adjusted analysis above for the optimal contrast,
