...
and $1_K$ is an all-ones vector of length $K$
[see @pinheiro2014 for a detailed account].
...
including the covariates to adjust for in the `addCovars` argument.
Another approach to perform the MCP step is to perform the MCP step
based on the differences to placebo: In a first stage `lm(.)` is fit
with an intercept included. Then the treatment differences and
corresponding covariance matrix would be extracted. This could then be
fed into the `MCTtest` function, with the `placAdj = TRUE` argument,
see `?MCTtest` for an example. Both approaches will give the same
result.
...
averaging over all patients per dose (for an identity link function
this is equivalent to prediction of a single patient with "average"
covariate values).
