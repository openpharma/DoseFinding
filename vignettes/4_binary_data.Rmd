...
Assume a dose-finding study is planned for an investigational treatment
with doses 0, 0.5, 1.5, 2.5, 4. It is assumed the response rate for placebo
will be around 10%, while the response rate for the top dose may be 45%.
...
Assume the following candidate set is desired to be used.
...
mods <- Mods(emax = c(0.25, 1), 
             ...
             betaMod = c(1.1, 1.1),
             placEff = logit(0.1), maxEff = logit(0.45)-logit(0.1),
         doses = doses)
...
In many situations there are important prognostic covariates (main effects) to adjust for in the analysis. The code below illustrates, how this can be achieved.
...
When re-running the adjusted analysis above for the optimal contrast, calculated as outlined above, we only a slightly different result.
```{r}
