...
We will consider the same example study and the same candidate models as
in the [vignette for analysis of normally distributed
...
![](sample_size_files/figure-html/setup-1.png)
...
First we calculate the matrix of optimal contrasts (`w=1` denotes
homoscedastic residuals with equal group sizes, see
...
We request five equally sized groups with `alRatio = rep(1, 5)`. We fix
the residual standard deviation with `sigma = 0.34`, and calculate the
...
![](sample_size_files/figure-html/power_sample_size_1-1.png) This shows
the power values of the maximum contrast test assuming each of the
different candidate models to be true. The minimum, mean and maximum
power over the candidate models are also included in the plot.

There also is a wrapper function that calculates the group sample sizes
needed in order to attain a specific power. The powers under each
alternative model are combined with `sumFct`. Here we look at the
...
treatment Effect `maxEff`. Note how power decreases if we assume a
...
![](sample_size_files/figure-html/power_effect_size-1.png)

## Power under mis-specification

MCP-Mod depends on the candidate models selected. What if the true model
is not among the chosen candidate shapes? Often MCP-Mod is rather
...
![](sample_size_files/figure-html/power_miss_1-1.png)
...
![](sample_size_files/figure-html/power_miss_2-1.png)
...
The main purpose of a dose-finding study is selection of a dose to take
forward into Phase 3. Establishment of a trend over placebo is hence
only a minimum requirement before considering dose-selection.
...
it turns out that this is a much harder problem than establishing a
dose-response effect versus placebo based on the MCP-part: The sample
size required for adequate accuracy in estimation of a target dose
(e.g. the smallest dose achieving a relevant improvement over placebo)
is usually several-fold higher than the sample size needed to have
...
Here we only consider a brief example: Consider the `sigEmax(30.5, 3.5)`
model from the first section and assume that it is the “true model”
under which we want to investigate the operating characteristics of
fitting sigEmax models. Suppose we want to achieve a target improvement
of \\\Delta=0.12 L\\ over placebo. One can calculate that this needs a
target dose TD of 44.4 mg under the true model. Keep this number in mind
for later. Now we can ask the question what the variability in TD
estimation would be. To answer it, we can run a simulation using the
`planMod` function. If we use the sample size n=93 from the power
...
The output shows different outputs (see
[`?planMod`](https://openpharma.github.io/DoseFinding/reference/planMod.md)
for details) of most interest here is the length of the quantile range
for a target dose (`lengthTDCI`). By default this is calculated by
...
`P(no TD)` indicates in how many simulations runs no TD could be
identified.

From the output it can be seen that the variation in the TD estimates is
quite large and quite unsatisfactory. Experimenting with different
values of `n`, one quickly realizes that we would need for example 1650
...
tolerability or safety markers, will contribute to that decision.

## References

O’Quigley, John, Alexia Iasonos, and Björn Bornkamp. 2017a. *Handbook of
