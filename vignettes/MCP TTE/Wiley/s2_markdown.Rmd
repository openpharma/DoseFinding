...
p0=1/apply(exp(0.5*beta)*eta,2, sum)
pk=(exp(0.5*beta)*eta)%*%diag(p0)

##  S matrix
S=array(0,c(ngrp-1,ngrp-1,nshape))
for (i in 1:nshape) {
  S[,,i]=1/p0[i]
...
model6 <- Mods(betaMod=betam1, doses=dose, placEff=log(lambda0), maxEff=(log(lambda1)-log(lambda0))) 
